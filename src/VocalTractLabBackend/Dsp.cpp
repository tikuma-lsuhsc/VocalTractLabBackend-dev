...
void getPolynomRoots(double *a, int &N, ComplexValue *roots)
{
  ...
  int l, j, i, m;
  bool fertig = false;
  int E;                    // Z�hler f�r noch ungenaue Quadratfaktoren
  ...
  double startAngle[MAX_M]; // Wo liegt jeweils der 1. N�herungswert auf dem Einheitskreis ?


  // Falls der Polynomgrad N ungerade ist, dann eine weitere Nullstelle
  // durch Multiplikation des Polynoms mit x hinzuf�gen ***
  ...
  // Die komplexen Einheitswurzeln als Startn�herung vorgeben
  for (m=1; m <= M-1; m++)
  {
  ...
  double E1 = 0.0;
  double E2 = 0.0;
  const long MAX = 32;    // max. Anzahl der Iterationsschritte f�r alle Quadratfaktoren
  ...
  while ((l <= MAX) && (!fertig))
  {
    E = 0;                // Z�hler f�r die korrigierten Quadratfaktoren
    j = 1;
    ...
    while (j <= M)
    {
      ...
      // Weitere Korrektur ist noch n�tig
      if (fabs(c[0]) + fabs(c[1]) >= epsilon2)
      {
        ...
        // Determinante ist nahe Null => Startwerte ab�ndern und nochmal!
        if (fabs(D) < epsilon)
        {
          startAngle[j]+= 0.012345;  // Winkel auf komplexem Zahlenkreis um ein paar Grad erh�hen
          beta[j] = 2.0*cos(startAngle[j]);
          gamma[j] = 1.0;
          // j wird nicht erh�ht !
        }
        else
        // Die Korrekturen f�r den j-ten Quadratfaktor k�nnen berechnet werden
        {
          h = (c[0]*(S - u*T) - T*c[1]) / D;
          ...
          j++;    // Auf zum n�chsten Quadratfaktor
        }
      }   // weitere Korrektur war n�tig
      else { j++; }
