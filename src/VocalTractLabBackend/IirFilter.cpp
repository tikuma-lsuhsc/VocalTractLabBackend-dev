...
bool IirFilter::combineWithFilter(const IirFilter *f, bool cascade)
{
  ...
  // Umwandlung der Rekursionskoeffizienten in eine �bertragungsfunktion.
  ...
  // �bertragungsfunktion in Rekursionskoeffizienten verwandeln.
...
void IirFilter::createChebyshev(double cutoffFreqRatio, bool isHighpass, int numPoles)
{
  ...
  double a0, a1, a2, b1, b2;    // Koeff., die f�r jede 2-Pol-Stufe des Filters berechnet werden
  double t, w, m, d, k;
  ...
  order = numPoles;     // Die Ordnung des Filters in die Klassenglobale Variable �bernehmen
  ...
  for (pol=1; pol <= numPoles/2; pol++)
  {
    ...
    // Unterprogramm, welches f�r jede 2-Pol-Stufe des
    // Filters ausgef�hrt wird.
    ...
    // �berf�hrung vom s- in den z-Bereich
    t = 2.0*tan(0.5);
